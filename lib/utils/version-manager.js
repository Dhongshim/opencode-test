...
class VersionManager {
  ...
  static async createVersionFile(targetDir, version, options = {}) {
    const versionFile = path.join(targetDir, '.opencode', '.framework-version');
    const versionData = {
      version,
  ...
  static async updateVersionFile(targetDir, newVersion, oldVersion, options = {}) {
    ...
    const versionData = {
      ...
      updateSource: 'opencode-test-cli'
    };
    ...
    try {
      ...
      if (!compatible) {
        ...
        return {
          ...
          suggestion: 'Run "opencode-test update" to sync versions'
        };
      }
   ...
   */
  static async checkRemoteVersion(packageName = 'opencode-test') {
    try {
