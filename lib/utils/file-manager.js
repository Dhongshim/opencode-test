const fs = require('fs-extra');
const path = require('path');
const chalk = require('chalk');

class FileManager {
  /**
   * Check if framework exists in the given directory
   * @param {string} targetDir - Target directory path
   * @returns {boolean} - True if framework exists
   */
  static async isFrameworkExists(targetDir) {
    const opencodeDir = path.join(targetDir, '.opencode');
    const versionFile = path.join(opencodeDir, '.framework-version');

    return (
      (await fs.pathExists(opencodeDir)) && (await fs.pathExists(versionFile))
    );
  }

  /**
   * Copy framework files from template to target directory
   * @param {string} targetDir - Target directory path
   * @param {Object} options - Copy options
   */
  static async copyFrameworkFiles(targetDir, options = {}) {
    // Use the template directory from the CLI package
    const templateDir = this._getTemplateDir();
    const opencodeDir = path.join(targetDir, '.opencode');

    // Ensure template directory exists
    if (!(await fs.pathExists(templateDir))) {
      throw new Error(
        'OpenCode framework directory not found. Please ensure you are running from a valid opencode-test project.',
      );
    }

    // Remove existing .opencode directory if it exists and force is enabled
    if ((await fs.pathExists(opencodeDir)) && options.force) {
      if (options.verbose) {
        console.log(chalk.gray('  Removing existing framework...'));
      }
      await fs.remove(opencodeDir);
    }

    // Create target directory if it doesn't exist
    await fs.ensureDir(opencodeDir);

    // Copy template files
    if (options.verbose && process.env.NODE_ENV !== 'test') {
      console.log(chalk.gray('  Copying framework files...'));
    }

    await this._copyWithProgress(templateDir, opencodeDir, options);
  }

  /**
   * Update framework files while preserving user customizations
   * @param {string} targetDir - Target directory path
   * @param {Object} options - Update options
   */
  static async updateFrameworkFiles(targetDir, options = {}) {
    // Use the template directory from the CLI package
    const templateDir = this._getTemplateDir();
    const opencodeDir = path.join(targetDir, '.opencode');

    // Ensure template directory exists
    if (!(await fs.pathExists(templateDir))) {
      throw new Error(
        'OpenCode framework directory not found. Please ensure you are running from a valid opencode-test project.',
      );
    }

    // Preserve certain files during update
    const preserveFiles = ['.framework-version', 'settings.local.json'];

    if (options.verbose && process.env.NODE_ENV !== 'test') {
      console.log(chalk.gray('  Updating framework files...'));
    }

    await this._copyWithExclusions(
      templateDir,
      opencodeDir,
      preserveFiles,
      options,
    );
  }

  /**
   * Remove framework from target directory
   * @param {string} targetDir - Target directory path
   */
  static async removeFramework(targetDir) {
    const opencodeDir = path.join(targetDir, '.opencode');

    if (await fs.pathExists(opencodeDir)) {
      await fs.remove(opencodeDir);
    }
  }

  /**
   * Create backup of current framework
   * @param {string} targetDir - Target directory path
   */
  static async createBackup(targetDir) {
    const opencodeDir = path.join(targetDir, '.opencode');
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const backupDir = path.join(targetDir, `.opencode-backup-${timestamp}`);

    if (await fs.pathExists(opencodeDir)) {
      await fs.copy(opencodeDir, backupDir);
      if (process.env.NODE_ENV !== 'test') {
        console.log(
          chalk.gray(`  Backup created: ${path.basename(backupDir)}`),
        );
      }
    }
  }

  /**
   * Check framework file integrity
   * @param {string} targetDir - Target directory path
   * @returns {Object} - Integrity check result
   */
  static async checkFrameworkIntegrity(targetDir) {
    const opencodeDir = path.join(targetDir, '.opencode');
    const commandsDir = path.join(opencodeDir, 'commands');
    const scriptsDir = path.join(opencodeDir, 'scripts');

    const result = {
      isValid: true,
      missingFiles: [],
      commandsCount: 0,
      scriptsCount: 0,
      extraInfo: {},
    };

    // Check essential directories and files
    const essentialPaths = [
      { path: opencodeDir, type: 'directory', name: '.opencode' },
      { path: commandsDir, type: 'directory', name: 'commands' },
      { path: scriptsDir, type: 'directory', name: 'scripts' },
      {
        path: path.join(opencodeDir, '.framework-version'),
        type: 'file',
        name: '.framework-version',
      },
    ];

    for (const item of essentialPaths) {
      if (!(await fs.pathExists(item.path))) {
        result.isValid = false;
        result.missingFiles.push(item.name);
      }
    }

    // Count files if directories exist
    if (await fs.pathExists(commandsDir)) {
      try {
        const commandFiles = await fs.readdir(commandsDir);
        result.commandsCount = commandFiles.filter((file) =>
          file.endsWith('.md'),
        ).length;
      } catch (error) {
        result.extraInfo.commandsError = error.message;
      }
    }

    if (await fs.pathExists(scriptsDir)) {
      try {
        const scriptFiles = await fs.readdir(scriptsDir);
        result.scriptsCount = scriptFiles.filter(
          (file) => file.endsWith('.js') && !file.endsWith('.test.js'),
        ).length;
      } catch (error) {
        result.extraInfo.scriptsError = error.message;
      }
    }

    return result;
  }

  /**
   * Fix framework integrity issues
   * @param {string} targetDir - Target directory path
   * @param {Object} options - Fix options
   */
  static async fixFrameworkIssues(targetDir, options = {}) {
    const integrity = await this.checkFrameworkIntegrity(targetDir);

    if (integrity.isValid) {
      return { fixed: false, message: 'No issues found' };
    }

    if (options.verbose && process.env.NODE_ENV !== 'test') {
      console.log(chalk.yellow('  Attempting to fix integrity issues...'));
    }

    // Attempt to restore missing files
    await this.updateFrameworkFiles(targetDir, options);

    // Re-check integrity
    const newIntegrity = await this.checkFrameworkIntegrity(targetDir);

    return {
      fixed: newIntegrity.isValid,
      message: newIntegrity.isValid
        ? 'Issues fixed successfully'
        : 'Some issues remain',
    };
  }

  /**
   * Copy files with progress indication
   * @private
   */
  static async _copyWithProgress(src, dest, options = {}) {
    // Copy commands directory
    const commandsSrc = path.join(src, 'commands');
    const commandsDest = path.join(dest, 'commands');
    if (await fs.pathExists(commandsSrc)) {
      await fs.copy(commandsSrc, commandsDest, { overwrite: true });
      if (options.verbose && process.env.NODE_ENV !== 'test') {
        console.log(chalk.gray('    commands'));
        const files = await fs.readdir(commandsSrc);
        for (const file of files) {
          console.log(chalk.gray(`    commands/${file}`));
        }
      }
    }

    // Copy scripts directory
    const scriptsSrc = path.join(src, 'scripts');
    const scriptsDest = path.join(dest, 'scripts');
    if (await fs.pathExists(scriptsSrc)) {
      await fs.copy(scriptsSrc, scriptsDest, { overwrite: true });
      if (options.verbose && process.env.NODE_ENV !== 'test') {
        console.log(chalk.gray('    scripts'));
        const files = await fs.readdir(scriptsSrc);
        for (const file of files) {
          console.log(chalk.gray(`    scripts/${file}`));
        }
      }
    }

    // Skip settings.local.json - it's local configuration, not a template
  }

  /**
   * Get the template directory path
   * @private
   */
  static _getTemplateDir() {
    // Use the templates directory in the CLI package
    const templatesDir = path.join(__dirname, '../templates');

    if (fs.existsSync(templatesDir)) {
      return templatesDir;
    }

    throw new Error(
      'Could not locate template directory. Please ensure opencode-test is properly installed.',
    );
  }

  /**
   * Find the project root directory (contains .opencode directory)
   * @private
   */
  static _findProjectRoot() {
    let currentDir = __dirname;

    // Walk up the directory tree to find .opencode directory
    while (currentDir !== path.dirname(currentDir)) {
      const opencodeDir = path.join(currentDir, '.opencode');
      if (fs.existsSync(opencodeDir)) {
        return currentDir;
      }
      currentDir = path.dirname(currentDir);
    }

    // If not found, try relative to the CLI installation
    const cliRoot = path.join(__dirname, '../..');
    const opencodeInCli = path.join(cliRoot, '.opencode');
    if (fs.existsSync(opencodeInCli)) {
      return cliRoot;
    }

    // Fallback to current working directory
    const cwd = process.cwd();
    const opencodeInCwd = path.join(cwd, '.opencode');
    if (fs.existsSync(opencodeInCwd)) {
      return cwd;
    }

    throw new Error(
      'Could not locate .opencode directory. Please run from a valid opencode-test project.',
    );
  }

  /**
   * Copy files with exclusions
   * @private
   */
  static async _copyWithExclusions(
    src,
    dest,
    excludePattern = [],
    options = {},
  ) {
    const items = await fs.readdir(src);

    for (const item of items) {
      if (excludePattern.includes(item)) {
        if (options.verbose && process.env.NODE_ENV !== 'test') {
          console.log(chalk.gray(`    Preserving: ${item}`));
        }
        continue;
      }

      const srcPath = path.join(src, item);
      const destPath = path.join(dest, item);
      const stat = await fs.stat(srcPath);

      if (stat.isDirectory()) {
        await fs.ensureDir(destPath);
        await this._copyWithExclusions(
          srcPath,
          destPath,
          excludePattern,
          options,
        );
      } else {
        await fs.copy(srcPath, destPath);
        if (options.verbose && process.env.NODE_ENV !== 'test') {
          console.log(chalk.gray(`    Updated: ${item}`));
        }
      }
    }
  }
}

module.exports = FileManager;
