...
class JSONReportGenerator {
  ...
  validateData(data) {
    ...
    if (!['test', 'suite'].includes(data.reportType)) {
      throw new Error(
        `Invalid reportType: ${data.reportType}. Must be 'test' or 'suite'`,
      );
    }
  }
  ...
  async generateReport(data) {
    ...
    const { reportType, reportData, config = {}, environment = {} } = data;
  ...
  generateTestReport(reportData, config, timestamp) {
    ...
    let reportContent;
    if (isMultiple) {
      reportContent = this.generateBatchTestReportContent(
        testCase,
        execution,
        config,
        timestamp,
      ...
      reportContent = this.generateSingleTestReportContent(
        testCase,
        execution,
        config,
        timestamp,
  ...
  generateSuiteReport(reportData, config, timestamp) {
    ...
    // 生成报告内容
    const reportContent = this.generateSuiteReportContent(
      suite,
      results,
      config,
      timestamp,
...
<body>
  ...
  getStatusColor(status) {
    switch (status) {
      ...
      case 'failed':
        return '#dc3545, #e74c3c';
      default:
        return '#6c757d, #8d9498';
    }
  }
