...
describe('SuiteReportGenerator Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    testDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'opencode-test-suite-reports-'),
    );
    // 确保目录存在
  ...
  afterEach(() => {
    ...
    if (testDir && fs.existsSync(testDir)) {
      try {
        ...
        // 忽略清理错误，避免影响测试结果
        console.warn(
          `Warning: Failed to clean up test directory ${testDir}:`,
          error.message,
        );
      }
    }
  ...
  describe('Real Suite Report Generation', () => {
    test('should generate comprehensive suite report for smoke tests', () => {
      const suiteData = {
        ...
        testCases: [
          ...
          {
            name: 'sort-optimized.yml',
            description: 'Product sorting with session optimization',
          },
          {
            name: 'product-details.yml',
            description: 'Product details navigation test',
          },
      ...
      const executionResults = [
        {
          ...
          validations:
            'User authentication, Cart functionality, Payment processing, Order confirmation',
        },
        {
          testName: 'sort-optimized.yml',
          description:
            'Product sorting functionality with session optimization',
          status: 'passed',
          steps: 13,
          duration: '15s',
        ...
        },
        {
          ...
          duration: '20s',
          features:
            'Product Navigation, Details Verification, Cart Integration',
          tags: ['smoke', 'product-details', 'session-optimized'],
          validations:
            'Product information accuracy, Navigation functionality, Cart operations',
        },
        {
          testName: 'sort.yml',
    ...
    test('should generate detailed suite report with step information', () => {
      ...
      const result = detailedGenerator.generateSuiteReport(
        suiteData,
        executionResults,
      );
    ...
    test('should automatically create report directory', () => {
      ...
      const result = autoGenerator.generateSuiteReport(
        suiteData,
        executionResults,
      );
