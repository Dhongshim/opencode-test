...
async function check(options = {}) {
  ...
  try {
    ...
    if (options.verbose) {
      spinner.stop();
      console.log(chalk.blue('🔍 Checking opencode-test framework status...'));
    ...
    if (!frameworkExists) {
      ...
      console.log(chalk.gray('Run "opencode-test init" to initialize the framework.'));
      return;
    }
    ...
    if (integrity.isValid) {
      ...
      if (options.fix) {
        ...
        if (fixResult.fixed) {
          ...
          console.log(chalk.gray('Try running "opencode-test update" to restore all files.'));
        }
      } else {
        console.log(chalk.gray(''));
        console.log(chalk.gray('Use --fix to attempt automatic repair, or run "opencode-test update".'));
      }
    }
    ...
    if (!compatibility.compatible) {
      console.log(chalk.gray('• Run "opencode-test update" to sync framework with CLI version'));
    }
    
    if (!integrity.isValid && !options.fix) {
      console.log(chalk.gray('• Run "opencode-test check --fix" to repair missing files'));
    }
    
    if (compatibility.compatible && integrity.isValid) {
      console.log(chalk.gray('• Framework is in good condition'));
      console.log(chalk.gray('• Check "opencode-test --help" for available commands'));
    }
    ...
    if (options.remote) {
      ...
      if (remoteVersion) {
        ...
        if (isUpdateAvailable) {
          ...
          console.log(chalk.gray('  Run "npm update -g opencode-test" to update CLI'));
        } else {
