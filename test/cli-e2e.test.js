...
const runE2ETests =
  process.env.RUN_E2E_TESTS === 'true' || process.env.NODE_ENV === 'test';
...
describe('CLI End-to-End Tests', () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'opencode-test-e2e-'));
  ...
  describe('package installation', () => {
    test('should install package globally', async () => {
      ...
      try {
        ...
        // Verify installation
        const output = execSync('opencode-test --version', {
          encoding: 'utf8',
          timeout: 10000,
        });
    ...
    test('should show help information', () => {
      ...
      const output = execSync('opencode-test --help', {
        encoding: 'utf8',
        timeout: 10000,
      });
  ...
  describe('framework initialization', () => {
    test('should initialize framework in empty directory', () => {
      ...
      const output = execSync('opencode-test init --verbose', {
        cwd: testDir,
        encoding: 'utf8',
        timeout: 30000,
      });
      ...
      expect(fs.pathExistsSync(path.join(testDir, '.opencode'))).toBe(true);
      expect(
        fs.pathExistsSync(path.join(testDir, '.opencode', 'commands')),
      ).toBe(true);
      expect(
        fs.pathExistsSync(path.join(testDir, '.opencode', 'scripts')),
      ).toBe(true);
      expect(
        fs.pathExistsSync(
          path.join(testDir, '.opencode', '.framework-version'),
        ),
      ).toBe(true);
    });
    ...
    test('should check framework status', () => {
      ...
      // Then check
      const output = execSync('opencode-test check', {
        cwd: testDir,
        encoding: 'utf8',
        timeout: 10000,
      });
  ...
  describe('error handling', () => {
    test('should handle invalid commands gracefully', () => {
      ...
      try {
        execSync('opencode-test invalid-command', {
          encoding: 'utf8',
          timeout: 10000,
        });
  ...
  describe('update functionality', () => {
    test('should handle update in dry-run mode', () => {
      ...
      const output = execSync('opencode-test update --dry-run', {
        cwd: testDir,
        encoding: 'utf8',
        timeout: 15000,
      });
  ...
  afterAll(async () => {
    ...
    try {
      ...
      execSync('npm uninstall -g opencode-test', {
        stdio: 'pipe',
        timeout: 30000,
      });
    } catch (error) {
