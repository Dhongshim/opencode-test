...
describe('VersionManager', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'opencode-test-version-'),
    );
  ...
  describe('getCurrentVersion', () => {
    ...
    test('should handle missing version file', async () => {
      await fs.remove(path.join(tempDir, '.opencode', '.framework-version'));
    ...
    test('should handle corrupted version file', async () => {
      await fs.writeFile(
        path.join(tempDir, '.opencode', '.framework-version'),
        'invalid json',
      );
  ...
  describe('getVersionInfo', () => {
    test('should return complete version information', async () => {
      ...
      expect(info).toHaveProperty('installSource', 'opencode-test-cli');
    });
  ...
  describe('updateVersionFile', () => {
    test('should update version file with new data', async () => {
      ...
      const updatedData = await fs.readJSON(
        path.join(tempDir, '.opencode', '.framework-version'),
      );
    ...
    test('should handle update when framework does not exist', async () => {
      ...
      await expect(
        VersionManager.updateVersionFile(tempDir, '1.3.0', '1.2.0'),
      ).rejects.toThrow();
    });
  });
