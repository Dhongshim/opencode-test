...
describe('FileManager', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'opencode-test-filemanager-'),
    );
    ...
    await fs.writeFile(
      path.join(sourceDir, 'commands', 'test.md'),
      '# Test Command',
    );
    await fs.writeFile(
      path.join(sourceDir, 'scripts', 'test.js'),
      '// Test Script',
    );
  ...
  describe('copyFrameworkFiles', () => {
    test('should copy framework files to target directory', async () => {
      const opencodeTarget = path.join(targetDir, '.opencode');

      // Create a mock template structure
      const templatesDir = path.join(
        __dirname,
        '../../lib/templates/.opencode',
      );
      await fs.ensureDir(templatesDir);
      await fs.ensureDir(path.join(templatesDir, 'commands'));
      await fs.ensureDir(path.join(templatesDir, 'scripts'));
      await fs.writeFile(
        path.join(templatesDir, 'commands', 'test.md'),
        '# Test Command',
      );
      await fs.writeFile(
        path.join(templatesDir, 'scripts', 'test.js'),
        '// Test Script',
      );
      ...
      expect(await fs.pathExists(opencodeTarget)).toBe(true);
      expect(await fs.pathExists(path.join(opencodeTarget, 'commands'))).toBe(
        true,
      );
      expect(await fs.pathExists(path.join(opencodeTarget, 'scripts'))).toBe(
        true,
      );
    ...
    test('should handle missing template directory', async () => {
      // Create a temporary directory far from any .opencode directory
      const tempRootDir = path.join(
        require('os').tmpdir(),
        'opencode-test-isolated',
      );
      ...
      FileManagerModule._getTemplateDir = () => {
        throw new Error(
          'Could not locate template directory. Please ensure opencode-test is properly installed.',
        );
      };

      try {
        await expect(
          FileManagerModule.copyFrameworkFiles(targetDir),
        ).rejects.toThrow('Could not locate template directory');
      } finally {
  ...
  describe('updateFrameworkFiles', () => {
    test('should update existing framework files', async () => {
      ...
      await fs.writeJSON(path.join(opencodeTarget, '.framework-version'), {
        version: '1.0.0',
      });
    ...
    test('should handle update when target does not exist', async () => {
      await expect(
        FileManager.updateFrameworkFiles('/non/existent/target'),
      ).rejects.toThrow();
    });
  });
  ...
  describe('createBackup', () => {
    test('should create backup of framework directory', async () => {
      ...
      await fs.writeFile(
        path.join(opencodeDir, 'test-file.txt'),
        'backup test',
      );

      await FileManager.createBackup(targetDir);

      // Check if backup directory was created (check for any .opencode-backup-* directory)
      const files = await fs.readdir(targetDir);
      const backupExists = files.some((file) =>
        file.startsWith('.opencode-backup-'),
      );
    ...
    test('should handle backup when framework does not exist', async () => {
      ...
      const backupsDir = path.join(targetDir, '.opencode-backups');
  ...
  describe('checkFrameworkIntegrity', () => {
    test('should validate complete framework structure', async () => {
      ...
      await fs.writeFile(
        path.join(opencodeDir, 'commands', 'run-yaml-test.md'),
        '# Test',
      );
      await fs.writeFile(
        path.join(opencodeDir, 'scripts', 'yaml-test-processor.js'),
        '// Test',
      );
      await fs.writeJSON(path.join(opencodeDir, '.framework-version'), {
        version: '1.0.0',
      });
  ...
  describe('fixFrameworkIssues', () => {
    ...
    test('should handle fix when framework does not exist', async () => {
      // Move to a directory that doesn't have .opencode
      const originalCwd = process.cwd();
  ...
  describe('isFrameworkExists', () => {
    test('should detect existing framework', async () => {
      ...
      await fs.ensureDir(path.join(opencodeDir, 'commands'));
      await fs.ensureDir(path.join(opencodeDir, 'scripts'));
      await fs.writeFile(path.join(opencodeDir, '.framework-version'), '1.0.0');
  ...
  describe('_copyWithProgress', () => {
    test('should copy files with verbose output', async () => {
      ...
      await fs.writeFile(
        path.join(sourceDir, 'ide', 'test.txt'),
        'should not be copied',
      );

      const consoleSpy = jest.spyOn(console, 'log').mockImplementation();

      await FileManager._copyWithProgress(sourceDir, destDir, {
        verbose: true,
      });

      // Check that allowed files were copied
      expect(
        await fs.pathExists(path.join(destDir, 'commands', 'test.md')),
      ).toBe(true);
      expect(
        await fs.pathExists(path.join(destDir, 'scripts', 'test.js')),
      ).toBe(true);
      // settings.local.json should NOT be copied (it's local configuration)
      expect(
        await fs.pathExists(path.join(destDir, 'settings.local.json')),
      ).toBe(false);
  ...
  describe('_copyWithExclusions', () => {
    test('should copy files while excluding specified patterns', async () => {
      ...
      await FileManager._copyWithExclusions(sourceDir, destDir, [
        'exclude.log',
      ]);
    ...
    test('should copy nested directories with exclusions', async () => {
      ...
      await FileManager._copyWithExclusions(
        sourceDir,
        destDir,
        ['exclude.log'],
        { verbose: true },
      ...
      expect(
        await fs.pathExists(path.join(destDir, 'nested', 'file.txt')),
      ).toBe(true);
      expect(await fs.pathExists(path.join(destDir, 'exclude.log'))).toBe(
        false,
      );
  ...
  describe('copyFrameworkFiles edge cases', () => {
    test('should handle force option when directory exists', async () => {
      const opencodeDir = path.join(targetDir, '.opencode');
      await fs.ensureDir(opencodeDir);
      await fs.writeFile(path.join(opencodeDir, 'existing.txt'), 'existing');
  ...
  describe('updateFrameworkFiles edge cases', () => {
    test('should handle preservation of user files', async () => {
      ...
      await fs.ensureDir(opencodeDir);
      await fs.writeFile(
        path.join(opencodeDir, 'user-file.txt'),
        'user content',
      );
