...
describe('Performance Benchmarks', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'opencode-test-perf-'));
  });
  ...
  describe('Initialization Performance', () => {
    test('should initialize framework within acceptable time limits', async () => {
      ...
      try {
        ...
        console.warn(
          'Init failed (expected in test environment):',
          error.message,
        );
      }
    ...
    test('should handle multiple concurrent initializations', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const dir = await fs.mkdtemp(
          path.join(os.tmpdir(), `opencode-test-concurrent-${i}-`),
        );
      ...
      console.log(
        `Concurrent init (5 processes) took ${totalDuration.toFixed(2)}ms`,
      );
  ...
  describe('Resource Cleanup', () => {
    test('should properly clean up resources after operations', async () => {
      ...
      console.log(
        `Handle change: ${afterHandles - beforeHandles}, Request change: ${afterRequests - beforeRequests}`,
      );
    });
  });
  ...
  describe('Stress Testing', () => {
    test('should handle rapid successive commands', async () => {
      ...
      console.log(
        `${iterations} commands took ${totalDuration.toFixed(2)}ms (avg: ${avgDuration.toFixed(2)}ms)`,
      );
    }, 120000);
  });
});
...
class PerformanceMonitor {
  ...
  measureMemory(name) {
    ...
    console.log(
      `🧠 ${name} Memory: ${(memory.heapUsed / 1024 / 1024).toFixed(2)}MB`,
    );
    return memory;
  }
}
