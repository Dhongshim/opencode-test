...
async function initFramework(targetDir) {
  return new Promise((resolve, reject) => {
    const child = spawn(
      'node',
      [path.join(__dirname, '../../bin/opencode-test.js'), 'init', '--verbose'],
      {
        cwd: targetDir,
        stdio: 'pipe',
      },
...
describe('Framework Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'opencode-test-framework-'),
    );
  });
  ...
  describe('framework structure validation', () => {
    test('should create complete framework structure', async () => {
      ...
      expect(await fs.pathExists(path.join(opencodeDir, 'commands'))).toBe(
        true,
      );
      expect(await fs.pathExists(path.join(opencodeDir, 'scripts'))).toBe(true);
    ...
    test('should create all required script files', async () => {
      ...
      const scriptsDir = path.join(tempDir, '.opencode', 'scripts');
  ...
  describe('framework file content validation', () => {
    ...
    test('should have valid version file format', async () => {
      const versionFile = path.join(tempDir, '.opencode', '.framework-version');
      const versionData = await fs.readJSON(versionFile);
  ...
  describe('framework persistence and updates', () => {
    test('should maintain user customizations during updates', async () => {
      ...
      const userFile = path.join(tempDir, '.opencode', 'user-config.json');
  ...
  describe('cross-platform compatibility', () => {
    test('should work with different path separators', async () => {
      ...
      const commandsPath = path.join(tempDir, '.opencode', 'commands');
      const scriptsPath = path.join(tempDir, '.opencode', 'scripts');
  ...
  describe('performance and scalability', () => {
    ...
    test('should handle large project directories', async () => {
      ...
      expect(result.code).toBe(0);
      expect(await fs.pathExists(path.join(largeDir, '.opencode'))).toBe(true);
    });
  });
});
