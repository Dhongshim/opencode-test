...
function runCLI(args, options = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(
      'node',
      [path.join(__dirname, '../../bin/opencode-test.js'), ...args],
      {
...
describe('CLI Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'opencode-test-cli-'));
  });
  ...
  describe('init command integration', () => {
    test('should initialize framework in empty directory', async () => {
      ...
      expect(
        await fs.pathExists(path.join(opencodeDir, '.framework-version')),
      ).toBe(true);
    });
    ...
    test('should reinitialize with force flag', async () => {
      ...
      // Create a test file
      const testFile = path.join(tempDir, '.opencode', 'test-file.txt');
  ...
  describe('check command integration', () => {
    ...
    test('should detect missing files', async () => {
      ...
      await fs.remove(path.join(tempDir, '.opencode', 'commands'));
    ...
    test('should handle framework not found', async () => {
      ...
      expect(
        result.stdout.includes('Framework not found') ||
  ...
  describe('update command integration', () => {
    ...
    test('should create backup when requested', async () => {
      const result = await runCLI(['update', '--backup', '--dry-run'], {
        cwd: tempDir,
      });
  ...
  describe('error handling', () => {
    test('should handle invalid command', async () => {
      ...
      expect(
        result.stdout.includes('Invalid command') ||
    ...
    test('should handle missing required arguments', async () => {
      ...
      expect(
        result.stdout.includes('Usage:') ||
          result.stdout.includes('help') ||
          result.stderr.includes('Missing command') ||
          result.code === 1,
      ).toBe(true);
    });
  });
