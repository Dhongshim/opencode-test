...
describe('Error Handling Scenarios', () => {
  ...
  beforeAll(() => {
    cliPath = path.join(__dirname, '../../bin/opencode-test.js');
  });

  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'opencode-test-error-'));
  });
  ...
  describe('Network Error Handling', () => {
    test('should handle network timeouts gracefully', async () => {
      ...
      VersionManager.checkRemoteVersion = jest
        .fn()
    ...
    test('should handle file descriptor limits', async () => {
      ...
      try {
        ...
        if (error.code === 'EMFILE' || error.code === 'ENFILE') {
          console.warn(
            'File descriptor limit reached (expected in some environments)',
          );
        } else {
          throw error;
        }
  ...
  describe('Concurrent Operation Handling', () => {
    test('should handle multiple concurrent init operations', async () => {
      ...
      console.log(
        `Concurrent operations: ${successCount} succeeded, ${failCount} failed`,
      );
  ...
  describe('Recovery and Cleanup', () => {
    test('should recover from partial operations', async () => {
      ...
      try {
        ...
        expect(await fs.pathExists(path.join(opencodeDir, 'scripts'))).toBe(
          true,
        );
        expect(
