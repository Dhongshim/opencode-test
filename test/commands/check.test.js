...
describe('check command', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'opencode-test-check-'));
  ...
  test('should check for remote updates when remote option is enabled', async () => {
    ...
    const mockCheckRemoteVersion = jest
      .spyOn(VersionManager, 'checkRemoteVersion')
    ...
    const hasVersionInfo = allCalls.some(
      (call) =>
        typeof call === 'string' &&
        (call.includes('1.3.0') || call.includes('Latest')),
    );
  ...
  test('should detect missing framework files', async () => {
    ...
    await fs.remove(
      path.join(tempDir, '.opencode', 'commands', 'run-yaml-test.md'),
    );
    ...
    const allCalls = consoleSpy.mock.calls.flat();
    const hasIssuesInfo = allCalls.some(
      (call) =>
        typeof call === 'string' &&
        (call.includes('Issues') ||
          call.includes('missing') ||
          call.includes('integrity')),
    );
  ...
  test('should fix integrity issues when fix option is enabled', async () => {
    // Remove a required file
    await fs.remove(
      path.join(tempDir, '.opencode', 'scripts', 'yaml-test-processor.js'),
    );
  ...
  test('should handle framework not found error', async () => {
    ...
    expect(
      consoleSpy.mock.calls
        .flat()
        .some((call) => typeof call === 'string' && call.includes('not found')),
    ).toBe(true);
  ...
  test('should handle remote check errors gracefully', async () => {
    ...
    await expect(check({ remote: true, verbose: true })).rejects.toThrow(
      'Network error',
    );
  ...
  test('should handle fix option when integrity issues exist', async () => {
    const mockCheckIntegrity = jest
      .spyOn(FileManager, 'checkFrameworkIntegrity')
      .mockResolvedValue({ isValid: false, missingFiles: ['test-file'] });
    const mockFixIssues = jest
      .spyOn(FileManager, 'fixFrameworkIssues')
  ...
  test('should handle remote version check when update available', async () => {
    ...
    const mockIsUpdateAvailable = jest
      .spyOn(VersionManager, 'isUpdateAvailable')
  ...
  test('should handle version info with updated field', async () => {
    const mockGetVersionInfo = jest
      .spyOn(VersionManager, 'getVersionInfo')
      .mockResolvedValue({
        version: '1.0.0',
        installedAt: '2023-01-01',
        updatedAt: '2023-06-01',
      });
    const mockFormatVersionDisplay = jest
      .spyOn(VersionManager, 'formatVersionDisplay')
      .mockReturnValue({
        current: '1.0.0',
        installed: '1/1/2023',
        updated: '6/1/2023',
  ...
  test('should handle version info with previous field', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('Previous Version:'),
    );
  ...
  test('should handle incompatible framework version', async () => {
    const mockCheckCompatibility = jest
      .spyOn(VersionManager, 'checkCompatibility')
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('opencode-test update'),
    );
  ...
  test('should show extra integrity info in verbose mode', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('setting1: value1'),
    );
  ...
  test('should handle fix option when issues cannot be resolved', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('Some issues could not be automatically fixed'),
    );
  ...
  test('should show fix suggestion when integrity invalid and not using fix option', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('Run "opencode-test check --fix"'),
    );
  ...
  test('should show good condition message when everything is fine', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('Framework is in good condition'),
    );
  ...
  test('should handle remote version check failure', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('Could not fetch remote version'),
    );
  ...
  test('should handle check command error without verbose', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('Use --verbose for detailed error information'),
    );
