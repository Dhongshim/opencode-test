...
describe('Cross-Platform Compatibility', () => {
  ...
  describe('Platform Detection', () => {
    test('should correctly detect platform', () => {
      expect(['win32', 'darwin', 'linux', 'freebsd', 'openbsd']).toContain(
        platform,
      );
  ...
  describe('File System Compatibility', () => {
    ...
    beforeEach(async () => {
      tempDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'opencode-test-platform-'),
      );
    });
  ...
  describe('Line Endings', () => {
    ...
    beforeEach(async () => {
      tempDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'opencode-test-lineend-'),
      );
    });
  ...
  describe('Character Encoding', () => {
    ...
    beforeEach(async () => {
      tempDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'opencode-test-encoding-'),
      );
    });
    ...
    test('should handle Unicode characters', async () => {
      ...
      const unicodeContent =
        '🚀 Émojis and àccénts work! 中文字符 русский العربية';
