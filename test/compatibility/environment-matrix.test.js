...
describe('Environment Matrix Compatibility', () => {
  ...
  describe('Node.js Version Compatibility', () => {
    test('should support current Node.js version', () => {
      ...
      expect(semver.satisfies(currentEnv.nodeVersion, requiredVersion)).toBe(
        true,
      );
    });
  ...
  describe('Shell Environment', () => {
    ...
    test('should handle environment variables correctly', () => {
      const testEnvVar = 'opencode_TEST_ENV_VAR';
      ...
      try {
        ...
        const result = execSync(
          `node -e "console.log(process.env.${testEnvVar})"`,
          {
            encoding: 'utf8',
  ...
  describe('File System Characteristics', () => {
    ...
    beforeEach(async () => {
      tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'opencode-test-env-'));
    });
    ...
    test('should handle maximum path lengths', async () => {
      ...
      try {
        ...
        console.log(
          `Long path failed at ${longPath.length} characters: ${error.message}`,
        );
      }
    });
    ...
    test('should handle special characters in file names', async () => {
      ...
      for (const fileName of specialChars) {
        try {
          ...
          console.warn(
            `Special character file name failed: ${fileName} - ${error.message}`,
          );
        }
      }
    });
  ...
  describe('Resource Limits', () => {
    test('should check available memory', () => {
      ...
      console.log(
        `System memory: ${memoryGB.toFixed(2)}GB total, ${(freeMem / (1024 * 1024 * 1024)).toFixed(2)}GB free`,
      );
    ...
    test('should check CPU capabilities', () => {
      ...
      if (cpus.length === 1) {
        console.warn(
          'Single-core system detected - performance tests may be slower',
        );
      }
    });
    ...
    test('should check disk space', async () => {
      try {
        ...
        const testFile = path.join(
          os.tmpdir(),
          `opencode-test-disk-${Date.now()}.tmp`,
        );
  ...
  describe('Network Connectivity', () => {
    test('should have basic network stack', () => {
      ...
      const hasLocalhost = Object.values(networkInterfaces)
        .flat()
        .some(
          (iface) => iface.address === '127.0.0.1' || iface.address === '::1',
        );
  ...
  describe('Security Context', () => {
    ...
    test('should have appropriate file permissions', async () => {
      const testFile = path.join(
        os.tmpdir(),
        `opencode-test-perms-${Date.now()}.tmp`,
      );
