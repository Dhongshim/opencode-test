...
Validation workflow:
1. **Use the automated processor**: Run `node .opencode/scripts/yaml-test-processor.js --suites --suite={suite} --env={env}` to process and validate the test suite
...
```bash
node .opencode/scripts/yaml-test-processor.js --suites --suite={suite} --env={env}
```
