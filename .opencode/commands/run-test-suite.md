...
Execution workflow:
1. **Use the automated processor**: Run `node .opencode/scripts/yaml-test-processor.js --suites` with appropriate parameters to get processed test suites
...
```bash
node .opencode/scripts/yaml-test-processor.js --suites --env={env} --tags={tags} --suite={suite}
```
...
- **STEP 2**: Generate report using: `node .opencode/scripts/gen-report.js --data=/path/to/suite-data.json`
...
```javascript
const { createAndSaveSuiteData } = require('./.opencode/scripts/create-report-data.js');
...
```javascript
const { quickCreateSuiteData } = require('./.opencode/scripts/create-report-data.js');
...
node .opencode/scripts/gen-report.js --data=suite-data.json
node .opencode/scripts/gen-report.js --data=quick-suite.json
node .opencode/scripts/gen-report.js --data=/absolute/path/to/suite-data.json
```
...
// node .opencode/scripts/gen-report.js --data=latest-suite.json
```
