...
class PerformanceMonitor {
  ...
  endTimer(name) {
    ...
    if (this.config.verbose) {
      this.log(
        `${name}: ${duration.toFixed(2)}ms, Memory: ${(result.memoryDelta.heapUsed / 1024 / 1024).toFixed(2)}MB`,
      );
    }
  ...
  analyzeResults(command, results) {
    ...
    this.log(
      `✅ ${command}: ${analysis.performance.duration.avg.toFixed(2)}ms avg (${successfulRuns.length}/${results.length} successful)`,
    );
  ...
  async generateMarkdownSummary(report, outputPath) {
    ...
    // Add table header
    lines.push(
      '| Command | Success Rate | Avg Duration | Min | Max | P95 | Memory Avg |',
    );
    lines.push(
      '|---------|--------------|--------------|-----|-----|-----|------------|',
    );
    ...
    for (const result of report.results.filter((r) => r.success)) {
      ...
      const avgMemory = (result.performance.memory.avg / 1024 / 1024).toFixed(
        2,
      );

      lines.push(
        `| \`${result.command}\` | ${successRate} | ${avgDuration}ms | ${minDuration}ms | ${maxDuration}ms | ${p95Duration}ms | ${avgMemory}MB |`,
      );
    }
    ...
    lines.push('', '---', `*Generated by opencode-test performance monitor*`);
  ...
  async runFullBenchmark() {
    ...
    const cliPath = path.join(__dirname, '../bin/opencode-test.js');
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'opencode-perf-'));
...
if (require.main === module) {
  ...
  monitor
    ...
    .then((report) => {
      ...
      console.log(
        `Success rate: ${report.summary.successfulCommands}/${report.summary.totalCommands}`,
      );
      console.log(
        `Average duration: ${report.summary.averageDuration.toFixed(2)}ms`,
      );
