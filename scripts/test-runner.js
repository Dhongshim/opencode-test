 ...
 * Test Runner Script
 *
 * Comprehensive test runner for the opencode-test CLI tool.
...
class TestRunner {
  ...
  async runUnitTests() {
    ...
    const result = await this.runCommand(
      'npm run test -- --testPathPattern='test/(commands|utils)'',
    );
  ...
  async runE2ETests() {
    if (!this.config.runE2E) {
      this.log(
        'E2E tests skipped (set RUN_E2E_TESTS=true to enable)',
        'warning',
      );
      return true;
    }
    ...
    const result = await this.runCommand(
      'npm run test -- --testPathPattern='test/cli-e2e.test.js'',
      {
  ...
  async runPerformanceTests() {
    if (!this.config.runPerformance) {
      this.log(
        'Performance tests skipped (set RUN_PERFORMANCE_TESTS=true to enable)',
        'warning',
      );
      return true;
    }
  ...
  async run() {
    ...
    if (allPassed) {
      ...
      this.log(
        `Some tests failed. Check the report for details. Duration: ${duration}s`,
        'error',
      );
    }
