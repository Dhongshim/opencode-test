...
function checkPackageJson() {
  ...
  // Check bin field
  if (!pkg.bin || !pkg.bin['opencode-test']) {
...
function checkBinExecutable() {
  ...
  const binPath = path.join(__dirname, '..', 'bin', 'opencode-test.js');

  if (!fs.existsSync(binPath)) {
    throw new Error('CLI executable not found: bin/opencode-test.js');
  }
  ...
  try {
    ...
    for (const filePattern of pkg.files) {
      ...
      if (!fs.existsSync(filePath)) {
        throw new Error(
          `File specified in package.json does not exist: ${filePattern}`,
        );
      }
    }
...
function testCLIInstallation() {
  ...
  try {
    ...
    // Test CLI commands
    const versionOutput = execute('opencode-test --version', { silent: true });
    if (!versionOutput.includes(pkg.version)) {
      throw new Error('CLI version command failed or returned wrong version');
    }

    const helpOutput = execute('opencode-test --help', { silent: true });
    if (
      !helpOutput.includes('init') ||
      !helpOutput.includes('update') ||
...
function checkDependencies() {
  ...
  try {
    ...
    console.log(
      chalk.yellow(
        '⚠️  Security audit found issues. Review before publishing.',
      ),
    );
