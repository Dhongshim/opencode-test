...
/**
 * Release script for opencode-test
...
function validateGitStatus() {
  ...
  const branch = execute('git rev-parse --abbrev-ref HEAD', {
    silent: true,
  }).trim();
  if (branch !== 'master') {
    console.error(
      chalk.red(`❌ Not on master branch. Current branch: ${branch}`),
    );
    console.log(
      chalk.yellow('Please switch to master branch before releasing.'),
    );
    process.exit(1);
  }
...
function verifyRelease(version) {
  ...
  setTimeout(() => {
    try {
      const npmVersion = execute(`npm view opencode-test version`, {
        silent: true,
      }).trim();
      if (npmVersion === version) {
        console.log(
          chalk.green(
            `✅ Release verified: opencode-test@${version} is available on NPM`,
          ),
        );
      } else {
        console.log(
          chalk.yellow(
            `⚠️  NPM version (${npmVersion}) doesn't match released version (${version})`,
          ),
        );
      ...
      console.log(
        chalk.yellow(
          '⚠️  Could not verify NPM release (this is normal for new packages)',
        ),
      );
    }
  }, 5000);
...
function printInstructions(version) {
  ...
  console.log(
    chalk.gray('1. Check GitHub Actions for release workflow completion'),
  );
  ...
  console.log(chalk.cyan(`   npm install -g opencode-test@${version}`));
  console.log(chalk.cyan('   opencode-test --version'));
  console.log(chalk.gray('4. Update any dependent projects'));
  console.log(
    chalk.gray('5. Announce the release if it contains significant changes'),
  );
}
...
function main() {
  ...
  try {
    ...
    console.log(chalk.blue('\n📦 NPM Publishing:'));
    console.log(
      chalk.yellow('Note: NPM publishing will be handled by GitHub Actions'),
    );
